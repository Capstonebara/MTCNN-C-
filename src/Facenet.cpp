...
void Facenet::datasetExtract(string dataset, string aligned)
{
	...
	// std::cout << imgspath << std::endl;
    if (imgspath.empty()) {
        std::cout << "No images found in aligned directory!" << std::endl;
        return;
    }
...
{
	if (img.empty()) {
        throw cv::Exception(0, "Empty input image", "featureExtract", __FILE__, __LINE__);
    }

    // Convert the image to floating point and normalize it
    cv::Mat fimg;
    img.convertTo(fimg, CV_32F);
    fimg = (fimg - 127.5f) / 128.0f;

    // Create input blob with the correct dimensions and normalization
    cv::Mat inputBlob = cv::dnn::blobFromImage(fimg, 1.0, cv::Size(160, 160), cv::Scalar(0, 0, 0), true, false);
